	...
	if level == 'C':
		for line in INFILE:
			...
			if line.find('Chromosome') ==-1 and line.find('Complete') ==-1:
				...
				temp = line.split(',')
				genomesList.append([temp[0], temp[8]])
...
for i in range (numGenomes):
	OUTFILE.write('{0}\t{1}\n'.format(genomesList[i][0],genomesList[i][1]))
